...
- 디자인 상태: 반복 상자 갤러리를 제거하고 150개 행동을 의미 오브젝트로 구분했습니다. 프롤로그의 행동 격자를 실제 소파, 커피테이블 위 열쇠·찻잔·사진, 책장 앞 선택물, 메일 모니터, 기억문 장치로 재배치했습니다. 좌우 실내벽·창·중앙 출입문과 가구 군집을 구성하고 문을 막던 콘솔을 측면으로 이동해 실제 플레이어 통로를 확보했습니다. 최종 모델·텍스처·사운드와 실제 퍼즐 조작 연출은 아직 필요합니다.
...
- 진행 안내: 첫 기억 공간에 `기억 1→2→3` 순서, 현재 단계, 다음 조사 대상과 완료 후 출구 안내를 상시 표시합니다. 프롤로그 목표는 유나→소파→열쇠→찻잔→사진→선택→예약 메일→기억문→다음 방의 실제 대상 이름을 따라 갱신됩니다.
- 다음 구현 범위: 1장 이후 조사 스테이션을 장별 실제 퍼즐 조작과 기억 공간 연출로 고도화.
- 최신 검증: Unity 6000.3.18f1에서 재설계한 `04_StoryRoute` 실제 씬을 생성했습니다. 프롤로그 첫 화면, 유나·소파 도달, 중앙 문 통과, 다음 방 게이트, 기억 공간 안내를 포함한 전체 EditMode 290/290, PlayMode 62/62 통과, Windows Development 빌드 성공, 새 빌드 런타임 스모크 `[Stage2Smoke] PASS`와 예외 0건을 확인했습니다.
