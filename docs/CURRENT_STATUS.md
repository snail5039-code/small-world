...
- 기능 기준 커밋: `812865b` (`fix(player): safely cancel interaction feedback`)
...
- UI 상태: `00_Boot`~`04_StoryRoute`의 목표·조사·대화·기록/인벤토리·저장·설정·로딩·일시정지 UI를 공통 카드·색상·버튼 체계로 통일했습니다. 본문 16px, 프롬프트 18px, 제목 22px 역할 기준과 한글 동적 폰트 fallback, 긴 문구 줄바꿈·자동 높이, 44px 버튼, 1280×720/1920×1080 안전영역 계약을 실제 씬에 적용했습니다.
- 다음 구현 범위: 장별 조사 스테이션을 실제 퍼즐 조작과 기억 공간 연출로 고도화.
- 최신 검증: Unity 6000.3.18f1에서 전체 UI 기준을 5개 실제 씬에 적용했습니다. 영문 placeholder·깨진 글자·최소 글자 크기·긴 한글 줄바꿈·버튼 containment·모달 입력 계약을 포함한 전체 EditMode 284/284, PlayMode 61/61 통과, Windows Development 빌드 성공, 새 빌드 런타임 스모크 `[Stage2Smoke] PASS`를 확인했습니다.
