...
- 기능 기준 커밋: `d36234a` (`feat(stage15): 전체 스토리 행동 씬 연결`)
...
- 디자인 상태: 외부 에셋 없이 각 방의 대표 공간·소품 실루엣, 재질, 조명과 행동 동선을 구분한 최소 컬러 그레이박스 단계입니다. 최종 아트·텍스처·사운드와 실제 퍼즐 조작 연출은 아직 필요합니다.
- 다음 구현 범위: 장별 조사 스테이션을 실제 퍼즐 조작과 기억 공간 연출로 고도화.
- 최신 검증: Unity 6000.3.18f1 일반 Editor Test Runner에서 EditMode 195/195, PlayMode 50/50 통과, Windows Development 빌드 성공, 런타임 스모크 PASS.
