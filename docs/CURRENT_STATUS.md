...
- 기능 기준 커밋: `2ea6d9c` (`fix(stage15): 스토리 방 Tab Esc 입력 복구`)
...
- 15단계 구현 현황: 프롤로그, 1장 `네 번째 자리`, 2장 `마지막 승강장`과 `04_StoryRoute` 장면 연결이 구현되었습니다.
- 최근 안정화: 현실방 출구, 저장 UI 커서/입력 복원, 소녀 충돌, 근접 조사, 다음 방 Tab 기록/Esc 일시정지 입력을 보강했습니다.
- 다음 구현 범위: 3장 `완벽한 하루`.
- 최신 검증: Unity 6000.3.18f1 일반 Editor Test Runner에서 EditMode 162/162 통과, Windows Development 빌드 성공, 런타임 스모크 PASS.
